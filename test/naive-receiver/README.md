...
3. Avoid using `makeAddr()` cheatcode:
    ...
    function setUp() public {
        ...
        ...
    ```
4. `vm.getNonce()` is an unsupported cheatcode. Delete it in `_isSolved()` function.
...
We already know that if a problem cannot be solved in one transaction, we can try to solve it in several transactions. We are going in this direction. By analogy with [Truster](https://github.com/igorganich/damn-vulnerable-defi-halmos/tree/master/test/truster#another-transaction), let's just add another transaction to **SymbolicAttacker**:
```solidity
function execute_tx() private {
    address target = svm.createAddress("target");
    bytes memory data;
...
...
```
This function is the only function of this contract. In addition, it will not be executed if `msg.sender` is not `pool`. Therefore, we can safely delete `FlashLoanReceiver` from **GlobalStorage**. And next to that, we optimize the **NaiveReceiverPool** logic. 
...
function flashLoan(IERC3156FlashBorrower receiver, address token, uint256 amount, bytes calldata data)
    ...
    returns (bool)
{
    vm.assume (address(receiver) == address(0xaaaa0006)); // receiver from setup
    ...
}
```
### _checkRequest
...
Let's remember that under the condition of the problem, we have the `player` private key, which we can use to sign forward transactions here. It turns out that the only `request.from` we can pass is the `player` address:
```solidity
function execute(Request calldata request, bytes calldata signature) public payable returns (bool success) {
...
...
### Another way
Before proceeding to the analysis of the counterexample, it is worth saying that a similar result could have been achieved in an easier way. Instead of sacrificing possible scenarios, we could simplify the invariant even more. We could try to "bite in small pieces" and find these invariant counterexamples independently of each other. 
...
```
And Halmos did find a scenario: it needs to be called from `multicall()`, which will allow us to withdraw on behalf of the `forwarder` (via `delegateCall()`), while withdrawing funds to an arbitrary `receiver`, emptying the balance of an arbitrary address, inserting it at the end of `data`. For this we needed the hint with `svm.createBytes()` earlier.
...
```
Since we crafted the request ourselves (we couldn't use calldata from Halmos for obvious reasons) - I just copied this request from [here](https://medium.com/@opensiddhu993/challenge-2-naive-receiver-damn-vulnerable-defi-v4-lazy-solutions-series-8b3b28bc929d) by [@siddharth9903](https://github.com/siddharth9903) :)
```javascript
...
2. Halmos is not a magic wand that will solve complex problems without some help. You can and should use simplifications and optimizations by analysing the logic of target contracts. Sometimes nothing will work without it. The main thing is to choose a heuristic successfully, because this is rewarded with successfully found vulnerabilities.
